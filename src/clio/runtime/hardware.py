...
"""
clio Hardware Detection — CPU, GPU, and memory probing.

Used by extraction subsystems to auto-pick a device for sentence-transformer
encoding, vision-LLM rasterization, and similar AI workloads, or to fall back
to CPU-only paths on hosts without a GPU.
...
Usage:
    ...
    if hardware.can_use_gpu(min_free_memory_gb=5.0):
        # use sentence-transformers / vision LLM / local inference with GPU
        ...
    else:
...
def set_logger(log_fn: Callable[[str], None]) -> None:
    """
    Set custom logger function for clio output.
...
class HardwareProfile:
    ...
    def _get_cpu_model(self) -> str:
        ...
        if system == "darwin":
            ...
            try:
                ...
                _log(
                    f"[clio.runtime.hardware WARNING] Failed to detect macOS CPU model: {sys.exc_info()[1]}"
                )
                pass
            ...
            try:
                ...
                _log(
                    f"[clio.runtime.hardware WARNING] Failed to detect Linux CPU model: {sys.exc_info()[1]}"
                )
                pass
    ...
    def _detect_memory_macos(self) -> MemoryInfo:
        ...
        try:
            ...
            _log(
                f"[clio.runtime.hardware WARNING] Failed to detect macOS memory: {sys.exc_info()[1]}"
            )
    ...
    def _detect_memory_linux(self) -> MemoryInfo:
        ...
        try:
            ...
            _log(f"[clio.runtime.hardware WARNING] Failed to run free command: {sys.exc_info()[1]}")
            pass
        ...
        try:
            ...
            _log(
                f"[clio.runtime.hardware WARNING] Failed to detect Linux memory from /proc/meminfo: {sys.exc_info()[1]}"
            )
